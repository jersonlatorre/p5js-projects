class SimplexNoise {
  ...
  init() {
    for (let i = 0; i < 256; i++) {
      this.p[i] = Math.abs(~~(this.seed(i + this.rootSeed) * 256))
    }
  ...
  noise(x, y, z, w) {
    ...
    for (let i = 0; i < this.iOctaves; i++) {
      ...
      switch (arguments.length) {
        case 4:
          this.fResult += this.fPers * this.noise4D(this.fFreq * x, this.fFreq * y, this.fFreq * z, this.fFreq * w)
          break
        case 3:
          this.fResult += this.fPers * this.noise3D(this.fFreq * x, this.fFreq * y, this.fFreq * z)
          break
        default:
          this.fResult += this.fPers * this.noise2D(this.fFreq * x, this.fFreq * y)
      }
    }
