...
SteeringBehaviours.arrive = function(agent, targetPosition, arriveDistance, arriveRange) {
  if (!arriveRange) arriveRange = 99999
  
  // let sqrDist = SteeringBehaviours.squaredDistance(targetPosition, agent.position)
  let distance = agent.position.copy().sub(targetPosition).mag()
  ...
  if (distance < arriveRange) {
    if (distance > arriveDistance) {
      ...
      let speed = map(distance, 0, arriveDistance, 0, agent.maxSpeed)
...
SteeringBehaviours.wander = function(agent, wanderFactor) {
  let wanderRadius = 1
  let wanderDistance = wanderFactor
...
SteeringBehaviours.separate = function(agent, neighbors, separateRange) {
  ...
  return desiredSeparate.limit(agent.maxSpeed)
}
