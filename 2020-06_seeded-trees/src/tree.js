class Tree {
  ...
  draw() {
    switch (this.state) {
      case TreeState.GROWING_UP: {
        ...
        this.drawBranch(
          createVector(0.5 * width, 0.665 * height),
          createVector(0, -1),
          this.LEVELS,
          this.LENGTH * FACTOR
        )
  ...
  drawBranch(position, direction, level, length) {
    ...
    if (this.LEVELS == level) {
        ...
        .copy()
        .add(direction.copy().mult(this.tGrow * length * 1.8 * seed1))
    } else {
      end = position
        .copy()
        .add(direction.copy().mult(this.tGrow * length * seed1))
    }
    ...
    this.drawBranch(
      ...
      dir1,
      level - 1,
      length * seed1 * this.tGrow
    )
    this.drawBranch(
      createVector(end.x, end.y),
      dir2,
      level - 1,
      length * seed2 * this.tGrow
    )
  }
}
