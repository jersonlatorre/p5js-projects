...
class Sketch3 {
  ...
  initialize() {
    ...
    poisson = new PoissonDiskSampling({
      shape: [1080, 1080, 1080],
      minDistance: 1080 * 0.2,
      maxDistance: 1080 * 0.2,
      tries: 30,
    })
    ...
    pdPoints.forEach((pdPoint) => {
      ...
      points.push(p)
      this.dots.push(new Dot(index, p.x, p.y, p.z))
      index++
      // }
      // }
    })
...
class LineDrawer {
  ...
  draw() {
    ...
    if (this.isFinished) {
      graphics.line(this.p1.x, this.p1.y, this.p1.z, this.p2.x, this.p2.y, this.p2.z)
    } else {
      let t = sqrt(this.t)
      ...
      graphics.line(this.start.x, this.start.y, this.start.z, this.end.x, this.end.y, this.end.z)
    }
  }
}
